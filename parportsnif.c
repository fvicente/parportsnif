...
#define PP_VERSION "parportsnif: user-space parallel port sniffer"
...
static int pp_do_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if ((pp->flags & PP_CLAIMED) == 0) {
		pr_debug(CHRDEV "%x: claim the port first! CMD=[%d]\n", minor, cmd);
		return -EINVAL;
	}
...
{
	device_create(vpp_class, port->dev, MKDEV(VPP_MAJOR, port->number),
...
static int __init vpp_init (void)
{
	...
	if (register_chrdev (VPP_MAJOR, CHRDEV, &pp_fops)) {
